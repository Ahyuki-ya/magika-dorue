...
- `HERO_SPAWN_SEC` **25→5**、`HERO_SPAWN_SEC_FAST` **7→2**。待ち時間をなくし、絶え間なく勇者が来る流れへ。侵攻中の勇者が累積するので、深度100なら常時10体前後が坑道を降りてくる計算になる。
- `HERO_HP_MULT` **2.5→5**（1体を削る時間が倍）。
- **掘削SE を「ポクポク」に**: ノイズ主体の破砕音 → 520Hz サイン波の急速ピッチダウン（芯）＋260Hz 三角波の胴鳴り、ノイズはアタックに 0.012秒だけ。木を叩く丸い音。

### 連続流入の実測（`epilogue_tempo` に計測を追加・150秒・城の周囲に配置）
| 防衛戦力 | 討伐 | 同時に居た勇者 | 結果 |
|---|---|---|---|
...
通常モードは「Lv20なら捌ける／Lv10だと崩壊する」＝育成の意味がはっきり出た。
...
**対策案: `CASTLE_MAX_HP` 40 → 120**（早送り時の猶予が通常モードと同じ体感に揃う）。
2026-07-28 時点でユーザー判断により**一旦保留**。早送りを本格的に使う段階で再検討する。
