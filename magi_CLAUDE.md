...
- **薬** … 3段に分けた。
  1. `retreat` … 敵から離れる向きに歩く（`POTION_BACK_MS` = 1.6秒 または `POTION_SAFE` = 4マス離れるまで）
  2. `drink` … `POTION_DRINK_MS` = 1.2秒 立ち止まる
  3. 効き目 … `POTION_REGEN_MS` = 4.5秒 かけて `maxHp × POTION_HEAL`(0.5) をじわじわ足す。
     **一気に回復しないので、飲んだあとも詰められれば死ぬ**（緊迫感が残る）。
     HPは整数のまま保ち、端数は `regenAcc` に繰り越す
